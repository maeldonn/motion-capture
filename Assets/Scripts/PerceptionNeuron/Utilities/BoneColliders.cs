...
namespace Neuron
{
    public static class BoneColliders
    {
        ...
        public static void AddSkeletonColliders(Animator animator, string prefab_path, string prefab_path2, int layer)
        {
            ...
            if (template == null)
            {
                template = UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>(prefab_path2);
                if (template == null)
                {
                    Debug.LogError(string.Format("[NeuronBoneColliders] Can not load asset \"{0}\" and {1}", prefab_path, prefab_path2));
                    return;
                }
            }
        ...
        public static void AddSkeletonColliders(Transform root, string prefix, string prefab_path, string prefab_path2, int layer)
        {
            RemoveSkeletonColliders(root, prefix);

            // validate input
            Transform[] dest_bones = new Transform[(int)NeuronBones.NumOfBones];
            int bound_count = NeuronHelper.Bind(root, dest_bones, prefix, false, NeuronBoneVersion.NotSet_AutoByName);
            ...
            bound_count = NeuronHelper.Bind(src_object.transform, src_bones, "Robot_", false, NeuronBoneVersion.NotSet_AutoByName);
            if (bound_count < (int)NeuronBones.NumOfBones)
            {
        ...
        public static void AddSkeletonColliderRenderers(Animator animator, Color color)
        {
            ...
            if (mat == null)
                mat = UnityEditor.AssetDatabase.LoadAssetAtPath<Material>("Assets/NoitomNeuron/Neuron/Resources/Materials/BoneColliderMaterial.mat");
            if (mat != null)
            {
                mat.color = color;
            }
