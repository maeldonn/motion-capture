...
namespace Neuron
{
    ...
    {
        public static string GetBoneName(int id, bool isRigV2)
        {
        ...
        public static int GetBoneIndex(string name, int forceRigVersion = -1)
        {
            for (int i = 0; i < (int)NeuronBones.NumOfBones; ++i)
            {
                if (forceRigVersion == -1)
                {
                    if ((i == (int)Neuron.NeuronBones.Spine3 || i == (int)Neuron.NeuronBones.Neck))
                    {
                        if (name.EndsWith(GetBoneName(i, true), StringComparison.CurrentCultureIgnoreCase) || name.EndsWith(GetBoneName(i, false), StringComparison.CurrentCultureIgnoreCase))
                        {
                            return i;
                        }
                    }
                    else if (name.EndsWith(GetBoneName(i, false), StringComparison.CurrentCultureIgnoreCase))
                    {
                        return i;
                    }
                }
                else if (forceRigVersion == 1)
                {
                    if (name.EndsWith(GetBoneName(i, false), StringComparison.CurrentCultureIgnoreCase))
                    {
                        return i;
                    }
                }
                else if (forceRigVersion == 2)
                {
                    if ((i == (int)Neuron.NeuronBones.Spine3 || i == (int)Neuron.NeuronBones.Neck))
                    {
                        if (name.EndsWith(GetBoneName(i, true), StringComparison.CurrentCultureIgnoreCase))
                        {
                            return i;
                        }
                    }
        ...
        public static int Bind(Transform root, Transform[] bones, string prefix, bool verbose, NeuronBoneVersion boneVerson)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                //if ( t.name.StartsWith( prefix ) )
                {
                    ...
                    string bone_name = t.name;
                    int index = NeuronHelper.GetBoneIndex(bone_name.Trim(), (int)boneVerson);
                    //Debug.LogFormat("{0} {1} {2}", t.name, bone_name, index);
                    if (index >= 0)
                    {
                        if (bones[index] == null)
                            bones[index] = t;
        ...
        public static int AddBonesComponentsTransform<T>(Transform root, string prefix, OnAddBoneComponentTransform<T> add_delegate, params object[] args) where T : Component
        {
            ...
            int bound_count = Bind(root, bones, prefix, false, NeuronBoneVersion.NotSet_AutoByName);
