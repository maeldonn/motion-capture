...
namespace NeuronDataReaderManaged
{
    ...
    internal abstract class Session
    {
        ...
        private int reserveBufferSize = 4096;
        private int headerSize = 64;
        private int reserveSize = 0;
        private byte[] recvBuffer = null;
        private byte[] reserveBuffer = null;
        private IntPtr pinnedHeader = IntPtr.Zero;
        private IntPtr pinnedData = IntPtr.Zero;
        ...
        public void DoReceive()
        {
            while (!exitFlag)
            {
                ...
                // Buffer.BlockCopy(recvBuffer, 0, reserveBuffer, reserveSize, recvSize); // This line bug sometimes
                try
                {
        ...
        private BvhDataHeader AcquireHeader(byte[] buffer)
        {
        ...
        private void AcquireData(byte[] buffer, BvhDataHeader header)
        {
        ...
        private void SwapEndian(byte[] buffer, int offset, int size)
        {
            byte temp = 0;
            int end = size / 2 * 2;
        ...
        private bool ValidateHeader(BvhDataHeader header)
        {
    ...
    internal class TCPSession : Session
    {
        ...
        private IntPtr hashValue = IntPtr.Zero;
    ...
    internal class UDPSession : Session
    {
        ...
        private IPEndPoint sourceAddress = new IPEndPoint(IPAddress.Any, 0);
    ...
    {
        private static int MaxPacketSize = 32 * (64 + 60 * 6 * sizeof(float));
