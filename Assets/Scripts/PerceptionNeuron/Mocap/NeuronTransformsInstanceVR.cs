...
public class NeuronTransformsInstanceVR : NeuronInstance
{
    ...
    private float velocityMagic = 3000.0f;
    private float angularVelocityMagic = 20.0f;
    private List<NeuronBones> validBones = new List<NeuronBones>();
    private Transform rightArmTarget;
    private Transform RightArmHolder;
    ...
    private void SetRotation(NeuronBones bone, Vector3 rotation)
    {
    ...
    public bool Bind(Transform root, string prefix)
    {
        ...
        this.prefix = prefix;
        //int bound_count =
        NeuronHelper.Bind(root, bones, prefix, false, NeuronBoneVersion.NotSet_AutoByName);
    ...
    private void LoadValidBones()
    {
        switch (ActiveMode)
        {
            case Modes.RIGHT_ARM_FOLLOW_HMD:
                ...
                if (motionUpdateMethod == UpdateMethod.Normal)
                { // only detach in normal update mode
